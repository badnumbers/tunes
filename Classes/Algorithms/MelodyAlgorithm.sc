MelodyAlgorithm {
	*tonic { | chords, rhythm |
				...
				if (chords.isArray,
					{
						if (chords.size != 0,
							{
								if (chords.every({|alternatives|alternatives.class == Array}),
								{
										if (chords.every({|alternatives|alternatives.every({|chord|chord.class == Event})}),
											{
												if (chords.every({|alternatives|alternatives.every({|chord|chord.keys.includes('tonic')})}),
													{
														if (chords.every({|alternatives|alternatives.every({|chord|chord.tonic.isNil})}),
															{
															...
															{
																if (chords.every({|alternatives|alternatives.every({|chord|chord.tonic.isInteger})}),
																	{
																		if (chords.every({|alternatives|alternatives.every({|chord|chord.tonic >= 0})}),
																			{
																				if (chords.every({|alternatives|alternatives.every({|chord|chord.tonic <= 6})}),
																					{
												...
												if (chords.every({|alternatives|alternatives.every({|chord|chord.keys.includes('triad')})}),
													{
														if (chords.every({|alternatives|alternatives.every({|chord|chord.triad.isNil})}),
															{
															...
															{
																if (chords.every({|alternatives|alternatives.every({|chord|chord.triad.isArray})}),
																	{
																		if (chords.every({|alternatives|alternatives.every({|chord|chord.triad.size == 3})}),
																			{
																				if (chords.every({|alternatives|alternatives.every({|chord|chord.triad.every({|note|note.isInteger})})}),
																					{
																						if (chords.every({|alternatives|alternatives.every({|chord|chord.triad.every({|note|note >= 0})})}),
																							{
																								if (chords.every({|alternatives|alternatives.every({|chord|chord.triad.every({|note|note <= 6})})}),
																									{
											...
											{
												Error(format("The chords parameter passed to MelodyAlgorithm.tonic() must be an Array of Arrays of Events. The value % was received.", chords)).throw;
											},
										);
									},
									{
										Error("The chords parameter passed to MelodyAlgorithm.tonic() must be an Array of Arrays.").throw;
									}
								);
							},
							{
		...
		rhythm.dur.do({
			...
			currentNote = chords.wrapAt(currentChord)[0].tonic;
